...
![Azure Cosmos DB engineering guide to partition keys, request units, and consistency - Insight Crunch](/assets/images/blog/blog-75.webp)
...
The fastest way to make this concrete is to build it. You can [run the hands-on Azure labs and command library on VaultBook](https://vaultbook.net/tools/azure-labs.html) to create a container, choose a partition key, push synthetic load through it, and watch the per-partition RU consumption and hot-partition behavior in real time, which turns the model in this guide into something you have felt rather than only read.
