...
Making the configuration repeatable is the difference between a one-off and a production practice. The entire app definition expresses cleanly as infrastructure as code (Bicep, ARM, or Terraform), so the environment, the app, its scale rules, its ingress, and its identity and role assignments are version-controlled and reproducible rather than clicked together in the portal. The companion to this article is the place to run that end to end: you can [run the hands-on Azure labs and command library on VaultBook](https://vaultbook.net/tools/azure-labs.html) to deploy a container app, split traffic across two revisions, watch it scale to zero and back, and reproduce the registry-pull and target-port failures in a sandbox so the diagnosis is muscle memory before you meet it in production.
