---
layout: post
title: "SAT Statistics and Probability: Complete Data Analysis Guide"
page_title: "SAT Statistics and Probability Complete Guide: Mean, Median, Standard Deviation, Probability, Two-Way Tables, and Data Interpretation for the Digital SAT"
date: 2020-04-05
categories: ["Industry"]
...
![SAT Statistics and Probability Complete Guide](/assets/images/technology-industry-analysis-insightcrunch.webp)
...
1. [Measures of Center: Mean, Median, and Mode](#measures-of-center-mean-median-and-mode)
2. [Measures of Spread: Range and Standard Deviation](#measures-of-spread-range-and-standard-deviation)
3. [Reading and Interpreting Data Displays](#reading-and-interpreting-data-displays)
4. [Two-Way Frequency Tables](#two-way-frequency-tables)
5. [Probability: Basic, Conditional, and Independence](#probability-basic-conditional-and-independence)
6. [Scatter Plots and Lines of Best Fit](#scatter-plots-and-lines-of-best-fit)
7. [Data Collection Concepts](#data-collection-concepts)
8. [Margin of Error](#margin-of-error)
9. [Making Inferences from Sample Data](#making-inferences-from-sample-data)
...
Sum of six numbers = 6 times 15 = 90.
Sum of remaining five = 5 times 16 = 80.
Removed number = 90 - 80 = 10.
...
Sum of original 20 = 20 times 72 = 1440.
Sum of all 24 = 24 times 74 = 1776.
Sum of the 4 makeup students = 1776 - 1440 = 336.
Mean of 4 makeup students = 336 / 4 = 84.
...
2. If odd number of values: median is the ((n+1)/2)th value.
3. If even number of values: median is the average of the (n/2)th and ((n/2)+1)th values.
...
P(11th grade | basketball) = 20/95 ≈ 0.211 or 21.1%.
...
Two variables are independent if the conditional distribution of one variable is the same regardless of the value of the other. Check: P(basketball | 9th grade) = 45/80 = 56.25%. P(basketball | 10th grade) = 30/80 = 37.5%. These are different, so grade level and sport preference are NOT independent for this sample.
...
Divisible by 3: 3, 6, 9, ..., 48. Count = 16.
Divisible by 5: 5, 10, 15, ..., 50. Count = 10.
Divisible by both 3 and 5 (by 15): 15, 30, 45. Count = 3.
By inclusion-exclusion principle: 16 + 10 - 3 = 23.
Probability = 23/50.
...
P(first ace) = 4/52. After drawing one ace, 3 aces remain among 51 cards. P(second ace | first was ace) = 3/51. P(both aces) = (4/52) times (3/51) = 12/2652 = 1/221.
...
This is a conditional probability question: of the 96 people with good health, 72 exercise regularly. P(exercises | good health) = 72/96 = 3/4 = 0.75.
...
In practice with SAT data tables: P(A | B) = (count of outcomes that are both A and B) / (count of outcomes that are B).
...
P(English | Math) = count(passed both) / count(passed Math) = 8/18 = 4/9 ≈ 0.444.
...
Total who prefer reading = 80 + 60 = 140. P(tea | reading) = 80/140 = 4/7 ≈ 0.571.
...
P(cat | dog) = count(owns both) / count(dog owners) = 20/80 = 1/4 = 0.25.
...
**Test for independence:** A and B are independent if and only if P(A | B) = P(A). This is equivalent to saying P(A and B) = P(A) times P(B).
...
P(science) = 60/100 = 0.60. P(science | Grade 9) = 30/50 = 0.60. Since P(science | Grade 9) = P(science), grade and subject preference are independent for this data set.

**Testing independence from a table:** Compute P(A), then compute P(A | each category of B). If all these conditional probabilities equal P(A), A and B are independent. If they differ for any category, A and B are not independent.
...
Predicted grade: y = -2(5) + 100 = 90.
Residual = actual - predicted = 86 - 90 = -4.
...
1. Was the sample randomly selected? If not, no population inference is valid.
2. What population was the sample drawn from? Inferences apply only to that population.
3. Was this an observational study or experiment? Causal claims require an experiment.
4. Does the conclusion stay within the scope of the data, or does it overreach?
...
**Probability:** Calculate basic probability from counts. Use the complementary event rule. Apply conditional probability using appropriate denominators. Determine whether two events are independent by comparing P(A|B) to P(A). Calculate probability of two independent events occurring together.
...
Every statistical concept in this guide, from the simplest mean calculation to the most nuanced inference about study design, has been presented with the specific goal of building the knowledge and judgment that SAT statistics questions test. Students who work through all of these concepts methodically, who practice against official questions until each question type feels familiar, and who maintain the discipline to review every explanation carefully will find that statistics and data analysis becomes one of the most reliable and manageable sections of the SAT Math section. The concepts are learnable, the skills are buildable, and the questions are answerable. That is the promise of thorough, systematic preparation, and it is a promise this guide has been designed to deliver on for every student who applies its content with the focus and consistency that genuine mastery requires.
 Begin with the first topic in this guide today, build toward the last, and arrive at test day with the statistical literacy that the SAT's Problem-Solving and Data Analysis domain rewards. The work is defined, the resources are available, and the mastery is within reach.
 Statistics and probability knowledge, built carefully and applied consistently, produces score improvement that is as reliable as any preparation investment available for the SAT Math section. Invest in it fully and let the results reflect the quality of your preparation.
 That is the complete promise of this guide: thorough coverage, clear explanation, and the preparation framework needed to convert study time into reliable test day performance.
